...
Node::Node(std::string key) : key(key), val(0), color(NONE) { init(); }
Node::Node(std::string key, int val) : key(key), val(val), color(NONE) { init(); }
Node::Node(std::string key, int val, Color color) : key(key), val(val), color(color) { init(); }
void Node::init() {
  parent = nullptr;
  left_child = nullptr;
  right_child = nullptr;
...
std::shared_ptr<Node> BSTree::insertNode(const std::string &key, int val) {
  std::string pkey;
  ...
  auto queryLoc = findParentLoc(key);
  addChild(queryLoc.first, queryLoc.second, newNode);
